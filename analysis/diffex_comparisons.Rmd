...
bind_rows(Kallisto=p1,STAR=p2,.id="quant_type") %>%
  ggplot(aes(x=PC1,y=PC2,color=Response,shape=quant_type)) +
    geom_point() +
    scale_color_manual(values=RESPONSE_COLORS) + 
    labs(title="Superimposed PCA of RPM Normalized Counts from STAR/FC and Kallisto",
         shape="Method")
```
...
grid.newpage()
vd <- venn.diagram(x = list("Kallisto-Sleuth" = (sleuth.up),
                            "Kallisto-DESeq" = (deseq.k.up),
                            "FeatureCounts-DESeq" = (deseq.fc.up)),
...
grid.newpage()
vd <- venn.diagram(x = list("Kallisto-Sleuth" = (sleuth.down),
                            "Kallist-DESeq" = (deseq.k.down),
                            "FeatureCounts-DESeq" = (deseq.fc.down)),
                   main="Venn Diagram of Downregulated Genes Between 3 Methods",
                   fill = brewer.pal(4, "Set2")[1:3], filename = NULL)
grid.draw(vd)
